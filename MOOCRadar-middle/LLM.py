    ...
    def predict_from_excel(self, excel_file: str, name_col: str = 'name', 
        ...
        try:
            ...
            if output_file is None:
                ...
                output_file = f'prediction_results_{mode}_threshold{threshold_str}{start_part}.xlsx'
            
            if error_file is None:
                # 错误文件也添加起始行号
                error_part = f"_from{start_index}" if start_index > 0 else ""
                error_file = f'prediction_errors{error_part}.json'
...
def main():
    ...
    output_file = f"prediction_results_{mode}_threshold{threshold_str}{start_part}.xlsx"
    error_file = f"prediction_errors{start_part}.json"
    ...
    predictor = GPTKnowledgePredictor(
        analysis_top_k=10,  # 分析阶段考虑10个候选知识点
