# -*- coding: utf-8 -*-
"""
基于CoT推理路径的隐式知识点提取器 - Mooper数据集版本
...
class CoTKnowledgeExtractor:
    def __init__(self,
                 topics_file: str = "data/xlsx/topics.csv",
        ...
        print(f"初始化完成:")
        print(f"  - 知识点数量: {len(self.knowledge_points)}")
        print(f"  - LLM模型: {self.llm_model}")
        print(f"  - 相似度阈值 τ2: {self.similarity_threshold}")

    def _load_topics(self, file_path: str) -> None:
        """加载知识点CSV文件"""
        try:
            ...
            print(f"成功加载知识点文件: {file_path}")
        except Exception as e:
            print(f"加载知识点文件失败: {str(e)}")
...
注意：
- 每个步骤的premise应该是具体的编程知识点名称，如"JDBC连接"、"SQL查询"、"异常处理"等
- premise_set是所有步骤中涉及的知识点的去重集合
- 只输出JSON，不要输出其他内容"""

        user_message = f"""请对以下编程习题进行结构化链式思维推理：
...
{exercise_text}"""
    ...
    def potential_knowledge_mapping(self, 
        ...
        """
        premise_mappings = {}
        all_matched_knowledge = {}
        
        for premise in premise_set:
            if not premise:
                continue
                
            premise_vector = self._get_embedding(f"编程知识点：{premise}")
            ...
            for i, idx in enumerate(indices[0]):
                ...
                if similarity >= self.similarity_threshold:
                    ...
                    knowledge_name = self.knowledge_points.get(knowledge_id, "未知")
    ...
    def process_from_excel(self,
                           excel_file: str = "data/xlsx/filtered_results_with_summary.xlsx",
                           id_mapping_file: str = "data/exercise_id_mapping.json",
                           name_col: str = 'name',
        ...
        """
        从Excel文件批量处理习题，按exercise_id_mapping.json顺序
        
        Args:
            excel_file: 习题Excel文件路径
            id_mapping_file: 习题ID映射文件路径（定义处理顺序）
            name_col: 习题名称列名
            content_col: 习题内容列名
            output_file: 输出文件路径
            start_index: 起始索引
        ...
        try:
            ...
            print(f"成功读取习题文件: {excel_file}, 共 {len(input_df)} 条习题")
            
            # 建立 challenge_id -> row 的映射
            exercise_map = {}
            for idx, row in input_df.iterrows():
                challenge_id = str(row.get('challenge_id'))
                exercise_map[challenge_id] = row
            ...
            if os.path.exists(id_mapping_file):
                ...
                # 映射格式: {original_id: {original_id, new_id}}
                # 按 new_id 排序，获取 original_id 列表
                sorted_items = sorted(id_mapping.items(), key=lambda x: x[1]['new_id'])
                ordered_challenge_ids = [item[0] for item in sorted_items]
            else:
                print(f"未找到ID映射文件 {id_mapping_file}，按原始顺序处理")
                ordered_challenge_ids = [str(row.get('challenge_id')) for _, row in input_df.iterrows()]
            
            total_rows = len(ordered_challenge_ids)
            ...
            for i in tqdm(range(start_index, end_index), desc="处理进度"):
                challenge_id = ordered_challenge_ids[i]
                
                if challenge_id not in exercise_map:
                    print(f"警告: 索引 {i} 的 challenge_id '{challenge_id}' 在习题文件中不存在，跳过")
                    continue
                
                row = exercise_map[challenge_id]
                ...
                exercise_name = row[name_col]
                exercise_content = row[content_col]
                
                # 组合习题文本
                exercise_text = f"习题名称：{exercise_name}\n习题内容：{exercise_content}"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='基于CoT推理路径的隐式知识点提取器 - Mooper数据集版本'
    )
    parser.add_argument('--file', type=str, 
                        default="data/xlsx/filtered_results_with_summary.xlsx",
                        help='习题Excel文件路径')
    parser.add_argument('--mapping', type=str,
                        default="data/exercise_id_mapping.json",
                        help='习题ID映射文件路径（定义处理顺序）')
    ...
    parser.add_argument('--name-col', type=str,
                        default='name',
                        help='习题名称列名')
    parser.add_argument('--content-col', type=str,
                        default='summarized_content',
                        help='习题内容列名')
